...
bootstrap <- function(x, y){
  ...
  for (i in c(1:num_samples)){
    index <- sample(c(1:num_samples), 1)
    samp_x <- c(samp_x, x[index])
    samp_y <- c(samp_y, y[index])
  }
  return(list(samp_x, samp_y))
}
...
for (i in c(1:num_resamples)){
  ...
  boot_x_y <- bootstrap(grp_x, grp_y)
  boot_x <- boot_x_y[[1]]
  boot_y <- boot_x_y[[2]]
...
cat ("We have ", conf_interval * 100, "% confidence that the true r is between: ", out[lower_bound], "and ", out[upper_bound], "\n")
cat ("***** Bias Corrected Confidence Interval *****\n")
cat ("We have", conf_interval * 100, "% confidence that the true r is between: ", out[bias_corr_lower_bound], "and ", out[bias_corr_upper_bound], "\n")

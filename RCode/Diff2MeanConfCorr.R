...
for (i in c(1:num_resamples)){
  ...
  bootstrap_samples = list()  # list of vectors
...
lower_bound <- ceiling(num_resamples * (0.5 + sd_to_area(z_alpha_over_2 + (2 * z_0))))
upper_bound <-  floor(num_resamples * (0.5 + sd_to_area(z_1_minus_alpha_over_2 + (2 * z_0))))
...
cat ("\nWe have", conf_interval * 100, "% confidence that the true difference between the means is between: ", out[lower_bound], "and ", out[upper_bound])

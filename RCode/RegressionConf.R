...
for (i in c(1: num_resamples)){
  ...
  boot_regression <- regressionline(boot_x, boot_y)
  boot_a <- boot_regression[1]
  boot_b <- boot_regression[2]
...
cat("We have", conf_interval * 100, "% confidence that the true slope")

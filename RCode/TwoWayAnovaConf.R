...
for(i in c(1:num_resamples)){
  ...
  r <- 0
  c <- 0
  for (r in c(1:length(json_data))){
    bootstrap_samples <- lappend(bootstrap_samples, c())
    for (c in c(1:length(json_data[[r]]))){
      bootstrap_samples[[r]][[c]] <- bootstrap(json_data[[r]][[c]])
    }
  }
...
cat ("We have", conf_interval * 100, "% confidence that the true F-statistic")
